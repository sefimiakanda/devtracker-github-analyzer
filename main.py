...
def analyze_developer_data(user_data: dict, repos: list):
    ...
    for repo in repos:
        ...
        # Track the most popular repository based on star count
    ...
    # Formula for custom activity score
...
def main():
    ...
    # Display Results
    console.print(f"\n[bold green]Analysis Results for {username}:[/bold green]\n")
    console.print(f"Full Name: {user_data.get('name', 'N/A')}")
    console.print(f"Public Repositories: {user_data.get('public_repos', 0)}")
    console.print(f"Total Stars Received: {stats['total_stars']}")
    console.print(f"Total Forks: {stats['total_forks']}")
    console.print(f"Most Popular Repo: {stats['most_popular_repo']}")
    console.print(f"Top Languages: {', '.join(stats['top_languages']) if stats['top_languages'] else 'N/A'}")
    console.print(f"[bold cyan]Custom Activity Score: {stats['activity_score']}[/bold cyan]")
